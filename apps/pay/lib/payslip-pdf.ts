...
/** Generates a payslip PDF buffer + filename (shared by single download and download-all, Y2.1). */
export async function generatePayslipPdf(payslip: PayslipPdfInput): Promise<{ buffer: Buffer; filename: string }> {
  ...
  if (payslip.payType === "PERCENTAGE_SPLIT") {
    ...
    if (therapyMins > 0 && therapyRate > 0) {
      ...
      row(`Therapy (${therapyMins} mins @ £${therapyRate.toFixed(4)}/min)`, `-${gbp(therapyPence)}`);
    }
  } else {
