/**
 * Live Flow dashboard vs parity-exports/legacy-flow-stats.json
 * Usage: npx tsx scripts/compare-flow-live.ts
 */
    ...
    JSON.stringify(
      {
        ...
        livePounds: {
          ...current,
          totalPipelineValue: Math.round(current.totalPipelineValuePence / 100),
          totalPlanned: Math.round(current.totalPlannedPence / 100),
          totalPaid: Math.round(current.totalPaidPence / 100),
        },
        legacy: legacy.stats,
        ok: cmp.ok,
        diffs: cmp.diffs,
