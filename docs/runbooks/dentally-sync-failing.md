...
| Sync stuck `RUNNING` | Auto-clears after **2 hours** on status load (`STALE_RUNNING_MS`). Or `GET /api/cron/clear-stuck-dentally-sync?force=1` with Shell `CRON_SECRET`. Then Sync now. |
